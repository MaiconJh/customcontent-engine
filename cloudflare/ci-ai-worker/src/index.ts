...
import { readJsonBody, SecurityError, validateGovernancePayload, validateImplementationPayload, validateIssuePlanPayload, validatePayload, validateSharedSecret } from "./security";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      ...
      if (!["/v1/analyze/failure", "/v1/analyze/diff", "/v1/analyze/governance", "/v1/plan/issue", "/v1/implement/issue"].includes(url.pathname)) {
        return errorResponse("BAD_REQUEST", "Unsupported route.", 404);
      }
      await validateSharedSecret(request, env);
      if (url.pathname === "/v1/implement/issue") {
        const rawImplementation = await readJsonBody(request, env);
        const implementationPayload = sanitizeObject(validateImplementationPayload(rawImplementation, env), Number(env.MAX_MODEL_INPUT_CHARS || "50000")) as unknown as ImplementIssuePayload;
        if (!checkRateLimit(request, env, url.pathname, implementationPayload.repository, "workflow_dispatch")) {
          return errorResponse("RATE_LIMITED", "Rate limit exceeded.", 429);
        }
        return Response.json(await proposeIssueImplementation(implementationPayload, env));
      }
